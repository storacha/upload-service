...
export function ShardDetail({client, space, shard}: {client?: Client, space?: Space, shard: Link}) {
  ...
  const filecoinInfo = useSWR<FilecoinInfoSuccess | undefined>(
    filecoinInfoKey,
    {
      fetcher: async () => {
        ...
        const { out } = await client.capability.filecoin.info(
          claim.data.equals as PieceLink
        )
        if (out.error) {
    ...
    <div className="border border-hot-red rounded-2xl bg-white p-5 max-w-4xl">
      ...
      <div className="pb-5 font-mono text-sm overflow-hidden no-wrap text-ellipsis">
        {claim.isLoading ? (
          ...
          <>
            <span className="border-hot-red border-b-2 border-dotted">
              {claim.data.equals.toString()}
            </span>
            <CopyIcon text={String(claim.data.equals)} />
          </>
        ) : (
          'Unknown'
        )}
          ...
          filecoinInfo.data.aggregates.map(({ aggregate, inclusion }) => {
            ...
            return (
              <div key={aggregate.toString()}>
                ...
                <span className="border-hot-yellow border-b-2 border-dotted">
                  {aggregate.toString()}
                </span>
                <CopyIcon text={aggregate.toString()} />
                <H2 className="mt-5">
                ...
                <InclusionProof
                  proof={inclusion.subtree}
                  piece={Piece.fromLink(piece)}
                  style={proofStyle}
                />
              </div>
            )
          ...
          <ol className="list-decimal list-inside">
            {filecoinInfo.data.deals.map((d) => (
              <li key={d.aux.dataSource.dealID}>
                <a
                  href={`https://filfox.info/address/f0${d.provider}`}
                  target="_blank"
                  className="underline"
                >
                  f0{d.provider}
                </a>
                <span className="text-xs tracking-wider font-bold px-2 text-black font-mono inline-block">
                  @
                </span>
                <a
                  href={`https://filfox.info/deal/${d.aux.dataSource.dealID}`}
                  target="_blank"
                  className="underline"
                >
                  {d.aux.dataSource.dealID.toString()}
                </a>
              </li>
            ))}
          </ol>
...
function InclusionProof({
  ...
  for (const node of Proof.path(proof)) {
    ...
    top =
      right === 1n ? Proof.computeNode(node, top) : Proof.computeNode(top, node)
    height++
  }
      ...
      {archy(data)
        ...
        .map((line, i) => {
          ...
          if (line.indexOf(' ') === -1) {
            return (
              <div key={line}>
                <span className="border-purple-500 border-b-2 border-dotted">
                  {line}
                </span>
          ...
          return (
            <div key={line}>
              ...
              <span
                className={`${isPath ? 'opacity-100' : 'opacity-30'} ${
                  isPiece ? 'border-blue-600 border-b-2 border-dotted' : ''
                }`}
              >
                {label}
              </span>
              {isPiece && (
                <>
                  <PieceIcon className="opacity-60" />
                  <span className="text-sm opacity-60">Piece CID</span>
                </>
              )}
            </div>
          )
