...
describe('http', () => {
        ...
        verificationMethod: [
          {
            ...
            publicKeyMultibase:
              'z6MkvUM1Par5bDJtWHHa9dMx6eniRXo8eQdn7kL788xt9ipi',
          },
        ],
      })
    )
  ...
  it('should resolve allowed DID', async () => {
    const resolver = HTTPResolver.create(['did:web:alice.example.com'], {
      fetch: mockFetch,
    })
    const result = await resolver.resolveDIDKey('did:web:alice.example.com')
    assert.deepStrictEqual(result, {
      ok: ['did:key:z6MkvUM1Par5bDJtWHHa9dMx6eniRXo8eQdn7kL788xt9ipi'],
    })
  })

  it('should resolve allowed RegExp DID', async () => {
    const resolver = HTTPResolver.create([/did:web:[a-z]*\.example\.com/], {
      fetch: mockFetch,
    })
    const result = await resolver.resolveDIDKey('did:web:alice.example.com')
    assert.deepStrictEqual(result, {
  ...
  it('should return error for invalid DID document', async () => {
    ...
    for (const doc of invalidDocuments) {
      ...
      assert(
        result.error,
        `Expected error for document: ${JSON.stringify(doc)}`
      )
      assert.equal(result.error.name, 'DIDKeyResolutionError')
    }
  })
