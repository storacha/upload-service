...
export const test = {
  ...
  'should list replicas': async (assert, { replicaStore }) => {
    ...
    const results = Result.unwrap(await replicaStore.list({
      space: r0.space,
      digest: r0.digest
    }))
    ...
    for (const expectedReplica of [r0, r1]) {
      ...
      assert.equal(actualReplica?.cause.toString(), expectedReplica.cause.toString())
    }
  },
  'should set replica status': async (assert, { replicaStore }) => {
    ...
    const results0 = Result.unwrap(await replicaStore.list({
      space: r.space,
      digest: r.digest
    }))
    ...
    const results1 = Result.unwrap(await replicaStore.list({
      space: r.space,
      digest: r.digest
    }))
  ...
  'should fail to set replica status if not exists': async (assert, { replicaStore }) => {
