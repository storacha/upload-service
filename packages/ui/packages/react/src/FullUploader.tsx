...
function Uploading({
  ...
  uploadProgress: Record<string, ProgressStatus>
}): JSX.Element {
  return (
...
function Done({ dataCID, gatewayURL }: { dataCID?: AnyLink; gatewayURL: (cid: string) => string }): JSX.Element {
  ...
  return (
    <div className="flex flex-col items-center w-full">
      ...
      <a className="font-mono text-xs max-w-full overflow-hidden no-wrap text-ellipsis" href={gatewayURL(cid)}>
        {cid}
      </a>
      <div className="my-4">
        <button
          className="inline-block border border-black hover:bg-white hover:text-black text-black uppercase text-sm px-6 py-2 rounded-full whitespace-nowrap"
          onClick={() => {
            setFile(undefined)
          }}
        >
...
function UploaderConsole({ gatewayURL }: { gatewayURL: (cid: string) => string }): JSX.Element {
  ...
  switch (status) {
    ...
    case UploadStatus.Succeeded: {
      return <Done dataCID={dataCID} gatewayURL={gatewayURL} />
    }
...
function UploaderContents(): JSX.Element {
  ...
  if (status === UploadStatus.Idle) {
    return hasFile ? (
      <>
        <div className="flex flex-row">
          <div
            className="w-12 h-12 py-0.5 flex flex-col justify-center items-center bg-white text-black text-xs uppercase text-center text-ellipsis rounded-xs mr-4"
            title={file.type}
          >
        ...
        <div className="p-4">
          <button type="submit" className="inline-block border border-black hover:bg-white hover:text-black text-black uppercase text-sm px-6 py-2 rounded-full whitespace-nowrap" disabled={file === undefined}>
            Start Upload
          </button>
        </div>
      </>
    ...
    return (
      <>
        <UploaderConsole gatewayURL={(cid) => `https://${cid}.ipfs.w3s.link`} />
      </>
    )
  }
}

export interface FullUploaderProps {
...
export function FullUploader({ onUploadComplete, space, gatewayURL, kmsConfig }: FullUploaderProps): JSX.Element {
  ...
  useEffect(() => {
    if (isPrivateSpace && kmsConfig !== undefined) {
      setKmsConfig(kmsConfig)
    }
  }, [isPrivateSpace, setKmsConfig, kmsConfig])
    ...
    <W3Uploader as="div" onUploadComplete={onUploadComplete} defaultWrapInDirectory={true}>
          ...
          <>
            ...
            <div className="flex flex-row items-center mb-5">
              <label className="mr-4">
                <input type="radio" name="upload-type" checked={uploadType === UploadType.File} onChange={() => changeUploadType(UploadType.File)} /> File
              </label>
              <label className="mr-4">
                <input type="radio" name="upload-type" checked={uploadType === UploadType.Directory} onChange={() => changeUploadType(UploadType.Directory)} /> Directory
              </label>
              <label className="mr-4">
                <input type="radio" name="upload-type" checked={uploadType === UploadType.CAR} onChange={() => changeUploadType(UploadType.CAR)} /> CAR
              </label>
            </div>
          </>
        )}
        <div className={`relative h-80 mb-5 p-8 rounded-md bg-white/5 hover:border-black border-2 border-dashed border-black flex flex-col justify-center items-center text-center`}>
      ...
      {isPrivateSpace ? (
        <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4 mt-4 text-center lg:text-left">
          <div className="w-1/2">
            <h4 className="text-sm mb-2">Private Data</h4>
            <p className="text-xs">Files uploaded to this space are encrypted locally and never published to Filecoin.</p>
          </div>
          <div className="w-1/2">
            <h4 className="text-sm mb-2">Hot Storage Only</h4>
            <p className="text-xs">Once removed from hot storage, they are gone forever and cannot be recovered.</p>
          </div>
        </div>
      ) : (
        <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4 mt-4 text-center lg:text-left">
          <div className="w-1/2">
            ...
            <p className="text-xs">All data uploaded here will be available to anyone who requests it using the correct CID. Do not upload any private or sensitive information in an unencrypted form.</p>
          </div>
          <div className="w-1/2">
            <h4 className="text-sm mb-2">Permanent Data</h4>
            <p className="text-xs">Removing files will remove them from the file listing for your account, but that doesn’t prevent nodes on the decentralized storage network from retaining copies of the data indefinitely.</p>
          </div>
        </div>
      )}
    </W3Uploader>
