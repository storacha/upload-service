...
import { createMockGatewayService, createMockGatewayServer } from './mocks/private-gateway.js'
...
await describe('KMSCryptoAdapter', async () => {
  await describe('Unit Tests', async () => {
    ...
    await test('should encode and decode key references correctly', async () => {
      ...
      const originalKeyRef = 'projects/test-project/locations/us-central1/keyRings/test-ring/cryptoKeys/test-key/cryptoKeyVersions/1'
  ...
  await describe('Integration Tests', async () => {
    await test('should complete full encryption workflow with mocked private gateway', async () => {
      ...
      const { gatewayDID, spaceDID, issuer, publicKeyPem, keyPair, delegationProof } = fixtures
      ...
      service.space.encryption.key.decrypt = Server.provide(Space.KeyDecrypt, async (input) => {
        decryptCalled = true
        assert.strictEqual(input.capability.with, spaceDID)
        assert.strictEqual(input.capability.can, 'space/encryption/key/decrypt')
        
        // Get the encrypted key from the request
        const encryptedSymmetricKey = input.capability.nb.encryptedSymmetricKey
      ...
      const gatewayServer = await createMockGatewayServer(service, gatewayDID, 5555)
      ...
      try {
        ...
        const testBlob = new Blob([testFile], { type: 'application/octet-stream' })
        ...
        const kmsMetadata = /** @type {import('../src/types.js').KMSKeyMetadata} */ (encryptResult.metadata)
        ...
        const decryptConfigs = {
          ...
          resourceCID: /** @type {import('@storacha/upload-client/types').AnyLink} */ (/** @type {any} */ ('bafybeid')),
          issuer,
          audience: gatewayDID.did()
        }

        const decryptResult = await adapter.decryptSymmetricKey(encryptResult.encryptedKey, decryptConfigs)
    ...
    await test('should handle encryption setup errors gracefully', async () => {
      ...
      const gatewayServer = await createMockGatewayServer(service, gatewayDID, 5556)
    ...
    await test('should handle key decryption errors gracefully', async () => {
      ...
      service.space.encryption.key.decrypt = Server.provide(Space.KeyDecrypt, async () => {
        return Server.error({
          name: 'KeyNotFound',
      ...
      const gatewayServer = await createMockGatewayServer(service, gatewayDID, 5557)
